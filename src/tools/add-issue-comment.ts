...
export function register(pi: {
	...
	pi.registerTool({
		name: "git_add_issue_comment",
		label: "Add Issue Comment",
		description:
			"Add a comment to a GitHub issue. Shows a review prompt where the user can accept, edit, or cancel before the comment is posted.",
		...
		async execute(
			...
			try {
				const output = await runGh(
					["issue", "comment", String(params.number), "--body", result.body],
					cwd,
				);
				const url = output || "";
